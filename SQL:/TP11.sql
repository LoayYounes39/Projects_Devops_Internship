...
nuCi number(5) constraint fk_co_ci references Circuit(nuCi) ON DELETE CASCADE
);
...
commit;
INSERT INTO Pilote values (8, 'TRULLI', 'Jarno', 'italienne' );
...
ALTER Table Circuit_Distance ADD distance number(4);
UPDATE Circuit_Distance SET distance = 5303 where nuCi = 4;
...
commit;
//Car la valeur existe déjà 
//Q13 MODIFY CONSTRAINT 
ALTER TABLE CIRCUIT
MODIFY (paysCi constraint ci_circuit NOT NULL);
